...
import LeaderIntelligence from "./LeaderIntelligence";
    ...
    <div className="relative inline-block">
      <button
        ...
        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
          currentAssignment
            ? "bg-apc-pink text-white shadow-sm"
            : "bg-apc-green text-white animate-bounce shadow-lg shadow-apc-green/20"
        }`}
      >
        ...
        createPortal(
          ...
          >
            ...
            <div className="space-y-1 max-h-56 overflow-y-auto custom-scrollbar pr-1">
              {teamMembers.map((m) => {
                const isSelected =
                  currentAssignment?.usuarioIds?.includes(m.id) ||
                  currentAssignment?.usuarioId === m.id;
                return (
                  <label
                    key={m.id}
                  ...
                  >
                    ...
                    <div className="flex flex-col min-w-0">
                      <span
                        className={`text-[10px] font-bold uppercase truncate ${isSelected ? "text-apc-pink" : "text-slate-600"}`}
                      >
                        {m.name}
                      </span>
                      <span className="text-[7px] text-slate-400 font-black uppercase tracking-tighter truncate">
                        {m.role}
                      </span>
                    </div>
                  </label>
...
const LeaderDashboard: React.FC<LeaderDashboardProps> = ({ onViewProject }) => {
  ...
  const [qaTimeRange, setQaTimeRange] = useState<
  ...
  const handleCloseAnnouncement = (forcePermanent = false) => {
    if (forcePermanent || dontShowAgain) {
      localStorage.setItem("leader_intelligence_announcement_v2", "true");
    }
  ...
  const getQAMetrics = (userId: string) => {
    ...
    projects?.forEach((p) => {
      const qaComments =
        p.comentarios?.filter(
          (c) =>
            (c.isSystemEvent &&
              (c.text?.includes("APROBADO en [REVISIÓN QA") ||
                c.text?.includes("RECHAZADO en [REVISIÓN QA"))) ||
      ...
      qaComments.forEach((qaAction) => {
        ...
        if (priorEntry) {
          ...
          const actionIdx = sortedComments.findIndex(
            (c) => c.id === qaAction.id,
          );
          if (actionIdx > 0) {
        ...
        const isFelipe =
          qaAction.authorName?.toLowerCase().includes("felipe lópez") || false;
    ...
    const avgTime =
      countReviewed > 0 ? (totalHoursSpent / countReviewed).toFixed(1) : "0";
    ...
    return {
      ...
      checks,
      avgTime,
      totalCalificadas: approved + rejected + checks,
    };
  };
  ...
  const getOperativeAreaMetrics = (userId: string, areaName: string) => {
    ...
    const avgTime =
      completed > 0 ? (totalHoursSpent / completed).toFixed(1) : "0";
  ...
  const handleDownloadReport = () => {
    ...
    const reportData = generateAreaReport(
      projects,
      users,
      activeArea,
      memberFilter,
    ...
    downloadCSV(
      reportData,
      `Reporte_Actividades_${activeArea}_${new Date().toISOString().split("T")[0]}`,
    );
  };
  ...
  const availableAreas = useMemo(() => {
    ...
    if (user.department === "QA" || user.role === UserRole.Correccion)
      return ["QA"];
  ...
  const teamMembers = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      const hasRole = (usr: User, role: UserRole) =>
        usr.role === role || (usr.roles && usr.roles.includes(role));
  ...
  const areaProjects = useMemo(() => {
    ...
    if (activeArea === "QA") {
      filtered = projects.filter((p) => {
        ...
        const isInGlobalQA =
          stageStr.includes("REVISIÓN QA") || p.status === "QA";
        const hasCampaignQA =
          p.esCampana &&
          p.estadoPorArea &&
          Object.values(p.estadoPorArea).some((status) => status === "En QA");
        return isInGlobalQA || hasCampaignQA;
      });
    } else {
      filtered = projects.filter((p) =>
        p.areas_seleccionadas?.some(
          (area) => normalizeString(area) === normalizeString(activeArea),
        ),
      );
    }
    ...
    if (memberFilter !== "all") {
      filtered = filtered.filter((p) =>
        p.asignaciones?.some(
          (a) =>
            a.usuarioIds?.includes(memberFilter) ||
            a.usuarioId === memberFilter,
        ),
      );
    }
    ...
    areaProjects.forEach((p) => {
      ...
      const hasClientLink =
        ...
        p.comentarios?.some((c) =>
          c.text.includes("PRESENTACIÓN PARA CLIENTE"),
        );
      const isStandby =
        p.enStandby ||
  ...
  const projectsToDisplay = useMemo(() => {
    let items = currentSubTab === "to_delegate" ? splitProjects.toDelegate :
      ...
      items = [...items].sort((a, b) => {
        ...
        if (sortConfig.key === 'odt') {
          ...
          // Simplified for sorting - getting the first name
          const getResponsible = (p: Project) => {
            const assignment = p.asignaciones?.find(
                (a) => normalizeString(a.area) === normalizeString(activeArea)
            );
            if (assignment) {
              return users.find(u => assignment.usuarioIds?.includes(u.id) || assignment.usuarioId === u.id)?.name || '';
            }
            return '';
          };
          aVal = getResponsible(a);
          bVal = getResponsible(b);
        } else if (sortConfig.key === 'fecha') {
            const getFecha = (p: Project) => {
                const areaNorm = normalizeString(activeArea);
                const areaKey = Object.keys(p.fechasInternas || {}).find(
                    (k) => normalizeString(k) === areaNorm
                ) || activeArea;
                return p.fechasInternas ? p.fechasInternas[areaKey] : null;
            };
            aVal = getFecha(a) || '9999-12-31';
            bVal = getFecha(b) || '9999-12-31';
    ...
    return items;
  }, [currentSubTab, splitProjects, sortConfig, activeArea, users]);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
          Sincronizando Historial de Operaciones...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <header className="flex justify-between items-center border-b pb-6">
        <div className="flex flex-col gap-4">
          <div>
            <h1 className="text-2xl font-black uppercase tracking-tight">
              Control de Área: {activeArea}
            </h1>
            <p className="text-sm text-slate-500 font-bold uppercase tracking-widest mt-1">
              Supervisión Técnica Operativa (ISO 9001)
            </p>
          </div>
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="flex flex-col items-end">
            <label className="text-[9px] font-black text-slate-400 uppercase mb-1 mr-1">
              Filtrar por Colaborador
            </label>
            <select
              className="bg-white border rounded-xl px-4 py-2 text-xs font-bold outline-none shadow-sm focus:ring-2 focus:ring-apc-pink"
              value={memberFilter}
      ...
      {showAnnouncement && (
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden animate-fadeIn mb-8">
          {/* Fondo Decorativo */}
          ...
          <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start">
            <div className="bg-white/20 p-4 rounded-2xl backdrop-blur-sm shadow-inner flex-shrink-0">
              <svg
                width="32"
                height="32"
                viewBox="0 0 24 24"
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <h2 className="text-xl font-black uppercase tracking-tight mb-2">
                  ¡Nueva Herramienta de Inteligencia para Líderes!
                </h2>
                <button
              ...
              <p className="text-indigo-100 text-sm font-medium mb-4 max-w-3xl leading-relaxed">
                Hemos integrado un nuevo módulo de{" "}
                <span className="font-black text-white">
                  "Inteligencia y Reportes"
                </span>{" "}
                impulsado por IA para ayudarte a optimizar la gestión operativa
                de tu área y elevar los estándares de calidad ISO 9001.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/10 p-3 rounded-xl border border-white/10">
                  ...
                  <p className="text-[11px] font-bold text-white">
                    Genera reportes estratégicos personalizados basados en el
                    desempeño real de tu equipo.
                  </p>
                </div>
                <div className="bg-white/10 p-3 rounded-xl border border-white/10">
                  <p className="text-[10px] font-black uppercase tracking-widest text-indigo-200 mb-1">
                  ...
                  <p className="text-[11px] font-bold text-white">
                    Visualiza automáticamente tiempos de ciclo, tasas de
                    aprobación y reprocesos técnicos.
                  </p>
                </div>
                <div className="bg-white/10 p-3 rounded-xl border border-white/10">
                  <p className="text-[10px] font-black uppercase tracking-widest text-indigo-200 mb-1">
                  ...
                  <p className="text-[11px] font-bold text-white">
                    Identifica cuellos de botella y recibe recomendaciones
                    estratégicas para escalar tu área.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 flex flex-col md:flex-row md:items-center gap-6">
                <div className="flex items-center gap-4">
                  <button
                    onClick={() => {
                      setCurrentSubTab("intelligence");
                      handleCloseAnnouncement(true);
                    }}
                    className="bg-white text-indigo-700 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-lg"
                  >
                    Explorar Ahora
                  </button>
                  <button
                    onClick={() => handleCloseAnnouncement()}
                    className="text-indigo-200 hover:text-white text-[10px] font-black uppercase tracking-widest transition-all"
                  >
                    Entendido, cerrar
                  </button>
                </div>
                ...
                <label className="flex items-center gap-2 cursor-pointer group">
                  <div className="relative">
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`w-4 h-4 rounded border-2 transition-colors flex items-center justify-center ${dontShowAgain ? 'bg-white border-white' : 'border-white/30 group-hover:border-white/50'}`}>
                      {dontShowAgain && (
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" strokeWidth="4">
                          <path d="M20 6L9 17l-5-5" />
                        </svg>
                      )}
                    </div>
                  </div>
                  <span className="text-[10px] font-bold text-indigo-200 group-hover:text-white transition-colors uppercase tracking-wider">
      ...
      {activeArea === "QA" ? (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <h2 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
              <svg
                className="w-4 h-4 text-apc-pink"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {teamMembers.map((m) => {
              const load = projects.filter(
                (p) =>
                  p.asignaciones?.some(
                    (a) => a.usuarioIds?.includes(m.id) || a.usuarioId === m.id,
                  ) && p.status !== "Finalizado",
              ).length;
              ...
              return (
                <div
                  key={m.id}
                  className={`p-4 rounded-3xl border transition-all shadow-sm bg-white ${isMe ? "border-apc-green/20" : "border-slate-100"}`}
                >
                  <div className="flex justify-between items-start mb-3 pb-3 border-b border-slate-50">
                    <div>
                      <p
                        className={`text-xs font-black pr-2 ${isMe ? "text-apc-green" : "text-slate-800"}`}
                      >
                        {isMe ? "Mi Rendimiento" : m.name}
                      </p>
                      <span
                        className={`text-[8px] font-black uppercase ${
                          m.role.includes("Lider") ||
                          m.role === UserRole.Correccion
                            ? "text-apc-green"
                            : "text-slate-400"
                        }`}
                      >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="flex flex-col items-center justify-center bg-emerald-50/50 rounded-lg p-2 border border-emerald-100/50">
                      <svg
                        className="w-3 h-3 text-emerald-500 mb-1"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="3"
                      ...
                      <p className="text-[7px] text-emerald-600/70 font-black uppercase tracking-widest">
                        Aprobadas
                      </p>
                    </div>
                    <div className="flex flex-col items-center justify-center bg-rose-50/50 rounded-lg p-2 border border-rose-100/50">
                      <svg
                        className="w-3 h-3 text-rose-500 mb-1"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="3"
                      ...
                      <p className="text-[7px] text-rose-600/70 font-black uppercase tracking-widest">
                        Rechazadas
                      </p>
                    </div>
                    <div className="flex flex-col items-center justify-center bg-blue-50/50 rounded-lg p-2 border border-blue-100/50">
                      <svg
                        className="w-3 h-3 text-blue-500 mb-1"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="3"
                      ...
                      <p className="text-[7px] text-blue-600/70 font-black uppercase tracking-widest">
                        Check OK
                      </p>
                    </div>
                  </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <h2 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
              <svg
                className="w-4 h-4 text-apc-green"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="flex flex-col items-center justify-center bg-emerald-50/50 rounded-lg p-2 border border-emerald-100/50">
                      ...
                      <p className="text-[7px] text-emerald-600/70 font-black uppercase tracking-widest text-center">
                        Entregas a Tiempo
                      </p>
                    </div>
                    <div className="flex flex-col items-center justify-center bg-amber-50/50 rounded-lg p-2 border border-amber-100/50">
                      <svg
                        className="w-3 h-3 text-amber-500 mb-1"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="3"
                      ...
                      <p className="text-[7px] text-amber-600/70 font-black uppercase tracking-widest text-center">
                    ...
                    <div className="flex flex-col items-center justify-center bg-rose-50/50 rounded-lg p-2 border border-rose-100/50">
                      ...
                      <p className="text-[7px] text-rose-600/70 font-black uppercase tracking-widest text-center">
                        Rechazos de QA
                      </p>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {teamMembers.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className={`p-5 rounded-2xl border transition-all shadow-sm hover:shadow-md ${isMe ? "bg-apc-green/5 border-apc-green/20" : "bg-white border-slate-100"}`}
              >
                <div className="flex justify-between items-start mb-3">
                  <p
                    className={`text-xs font-black truncate pr-2 ${isMe ? "text-apc-green" : "text-slate-800"}`}
                  >
                    {isMe ? "Mi Carga (Líder)" : m.name}
                  </p>
                  <span
                    className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase ${
                      m.role.includes("Lider") || m.role === UserRole.Correccion
                        ? "bg-apc-green text-white"
                        : m.role === UserRole.QA_Opera
      ...
      {!localStorage.getItem('dismissedSortNoticeLeader') && (
        <div className="bg-indigo-50 border border-indigo-200 p-4 rounded-xl flex items-center justify-between mb-6 shadow-sm">
          <p className="text-xs font-bold text-indigo-900">
             ¡Nueva funcionalidad! Ahora puedes organizar tus tablas haciendo clic en los encabezados de columna.
          </p>
          <button onClick={() => {
              localStorage.setItem('dismissedSortNoticeLeader', 'true');
              window.location.reload(); // Simple way to re-render and hide it
          ...
          <div className="flex flex-wrap border-b border-slate-100 bg-slate-50/50">
            ...
            >
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-[9px] font-black ${
                  currentSubTab === "working"
                    ? "bg-apc-green text-white shadow-sm"
                    : "bg-slate-200 text-slate-500"
                }`}
              >
            ...
            >
              ...
              <span>Inteligencia y Reportes</span>
            </button>
          </div>

          {currentSubTab === "intelligence" ? (
            <div className="p-6 bg-slate-50">
              <LeaderIntelligence activeArea={activeArea} />
            </div>
          ) : (
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-400 uppercase text-[10px] font-black tracking-widest">
                <tr>
                  <th className="px-6 py-5 cursor-pointer hover:text-indigo-600" onClick={() => requestSort('odt')}>ODT / Empresa</th>
                  <th className="px-6 py-5 cursor-pointer hover:text-indigo-600" onClick={() => requestSort('responsable')}>Responsable Asignado</th>
                  <th className="px-6 py-5 cursor-pointer hover:text-indigo-600" onClick={() => requestSort('fecha')}>Fecha Entrega</th>
                  <th className="px-6 py-5 cursor-pointer hover:text-indigo-600" onClick={() => requestSort('estado')}>Estado de Fase</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {projectsToDisplay.length === 0 ? (
                  ...
                  projectsToDisplay.map((p) => {
                    ...
                    const currentAssignment = p.asignaciones?.find(
                      (a) =>
                        normalizeString(a.area) ===
                        normalizeString(currentAssignmentArea),
                    );
                    const isAssignedToMe =
                      currentAssignment?.usuarioIds?.includes(user?.id || "") ||
                      currentAssignment?.usuarioId === user?.id;
                    ...
                    const displayStatus =
                      p.status === "En revisión con cliente" || hasClientLink
                    ...
                    const deadline = p.fechasInternas ? p.fechasInternas[areaKey] : null;

                    return (
                      <tr
                        key={p.id}
                        className={`hover:bg-slate-50/80 transition-colors ${isAssignedToMe ? "bg-apc-green/5" : ""}`}
                      >
                        <td className="px-6 py-4">
                          <p className="font-mono font-black text-apc-pink text-xs">
                            {p.id}
                          </p>
                          <p className="text-[10px] font-bold text-slate-800 uppercase truncate max-w-[200px]">
                            {p.empresa}
                          </p>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-2 h-2 rounded-full ${currentAssignment ? "bg-apc-green shadow-sm" : "bg-apc-pink/30 animate-pulse"}`}
                            ></div>
                            <span
                              className={`text-xs font-bold uppercase ${currentAssignment ? "text-slate-600" : "text-apc-pink italic"}`}
                            >
                              {currentAssignment
                                ? users
                                    .filter(
                                      (m) =>
                                        currentAssignment.usuarioIds?.includes(
                                          m.id,
                                        ) ||
                                        currentAssignment.usuarioId === m.id,
                                    )
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`text-[9px] font-black px-2 py-1 rounded uppercase tracking-tighter ${displayStatus === "En revisión con cliente" ? "bg-purple-600 text-white shadow-sm" : p.status === "QA" ? "bg-apc-pink/10 text-apc-pink shadow-sm" : "bg-slate-100 text-slate-500"}`}
                          >
                            {displayStatus}
                          </span>
                          <div className="text-[7px] font-black text-slate-400 mt-0.5 uppercase">
                            {p.etapa_actual}
                          </div>
                          {p.category === "PARRILLA RRSS" &&
                            p.materiales &&
                            p.materiales.length > 0 && (
                              <div className="text-[8px] font-bold text-apc-pink mt-1 uppercase tracking-widest">
                                {
                                  p.materiales.filter(
                                    (m) => m.estado === "Aprobado/Publicado",
                                  ).length
                                }
                                /{p.materiales.length} Mats
                              </div>
                            )}
                        </td>
