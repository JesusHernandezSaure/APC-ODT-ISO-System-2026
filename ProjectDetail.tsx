...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project: initialProject, onBack }) => {
  ...
  const isTypingBriefRef = React.useRef(false);
      ...
      <div className={`bg-white rounded-3xl border ${project.deleted ? 'border-rose-200 bg-rose-50/30' : 'border-slate-100'} shadow-2xl overflow-hidden mb-8 transition-all`}>
        ...
        <div className="bg-white p-6 rounded-3xl border shadow-sm">
          <div className="flex justify-between items-center mb-6">
            ...
            {canEditBrief && (
              <button onClick={() => {
                isTypingBriefRef.current = false;
              ...
              }} className="px-4 py-2 bg-apc-green text-white text-[10px] font-black rounded-lg hover:bg-apc-green/80 transition-all uppercase tracking-widest cursor-pointer">
                Guardar Cambios
              </button>
            )}
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border shadow-sm">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-slate-800 text-sm flex items-center gap-2 uppercase tracking-widest">
                ...
                Materiales aprobados Exclusivamente para cuentas
                <span className="text-[10px] font-bold text-slate-400 font-mono">({project.delivery_history.length})</span>
              </h3>
              <button
                type="button"
                onClick={() => setShowDeliveryHistory(!showDeliveryHistory)}
                className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-black rounded-lg transition-all flex items-center gap-1.5 uppercase tracking-wider cursor-pointer"
              >
                {showDeliveryHistory ? 'Ocultar' : 'Desplegar / Ver historial'}
                <Icons.ChevronRight className={`w-3.5 h-3.5 transition-transform duration-200 ${showDeliveryHistory ? 'rotate-90' : ''}`} />
              </button>
            </div>

            {showDeliveryHistory && (
              <div className="space-y-3 mt-4 pt-4 border-t border-slate-100 animate-fadeIn">
