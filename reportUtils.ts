...
export const analyzeODTWorkflow = (project: Project) => {
  const areaMetrics: Record<
    string,
    { days: number; internalRejections: number; clientRejections: number }
  > = {};
  ...
  logs.forEach((log, index) => {
    ...
    if (entryMatch) {
      ...
      if (
        currentArea &&
        entryTime &&
        (OPERATIVE_AREAS as readonly string[]).includes(currentArea)
      ) {
        const diffDays =
          (logDate.getTime() - entryTime.getTime()) / (1000 * 60 * 60 * 24);
    ...
    if (
      logText.includes("CLIENTE:") &&
      (logText.includes("RECHAZADA") || logText.includes("CON correcciones"))
    ) {
      const returnMatch = logText.match(/Regresando a \[([^\]]+)\]/);
      if (returnMatch) {
        const targetArea = returnMatch[1];
    ...
    if (
      ...
      const endTime = project.fecha_finalizado
        ? new Date(project.fecha_finalizado)
        : new Date();
      const diffDays =
        (endTime.getTime() - entryTime.getTime()) / (1000 * 60 * 60 * 24);
      areaMetrics[currentArea].days += Math.max(0, diffDays);
    }
  });
 ...
 */
export const downloadMasterCSV = (
...
export const generateAreaReport = (
  ...
  if (areaNorm === "qa" || areaNorm === "médico" || areaNorm === "médica") {
    ...
    projects.forEach((p) => {
      ...
      qaComments.forEach((qaAction) => {
        if (
          memberFilter &&
          memberFilter !== "all" &&
          qaAction.authorId !== memberFilter
        ) {
          return;
        }
        ...
        const feedback = feedbackMatch
          ? feedbackMatch.split(". Regresando")[0]
          : "";
    ...
    return qaRows.sort(
      (a, b) =>
        new Date(b["Fecha de Acción"] as string).getTime() -
        new Date(a["Fecha de Acción"] as string).getTime(),
    );
  }
  ...
  if (memberFilter && memberFilter !== "all") {
    areaProjects = areaProjects.filter((p) =>
      p.asignaciones?.some(
        (a) =>
  ...
  areaProjects.forEach((p) => {
    const assignments =
      p.asignaciones?.filter((a) => normalizeString(a.area) === areaNorm) || [];
    const internalRejections =
      p.internal_qa_rejection_count || p.contadorCorrecciones || 0;
    ...
    const felipe = users.find((u) =>
      u.name.toLowerCase().includes("felipe lópez"),
    );
    const isFelipeAssigned = assignments.some(
      (a) =>
        a.usuarioIds?.includes(felipe?.id || "") || a.usuarioId === felipe?.id,
    );

    const time = calculateWorkingTime(start, end, isFelipeAssigned);

    // Extract QA entry time from comments
    const qaEntryComment = p.comentarios?.find(
      (c) => c.isSystemEvent && c.text?.toUpperCase()?.includes("REVISIÓN QA"),
    );
    const qaExitComment = p.comentarios?.find(
      (c) =>
        c.isSystemEvent &&
        c.text?.toUpperCase()?.includes("APROBADO en [REVISIÓN QA]"),
    );

    let qaTime = { totalHours: 0 };
    if (qaEntryComment && qaExitComment) {
      qaTime = calculateWorkingTime(
        new Date(qaEntryComment.createdAt),
        new Date(qaExitComment.createdAt),
        isFelipeAssigned,
      );
      ...
      Integrantes: assignments
        .map((a) => {
          const assignedUsers = users.filter(
            (u) => a.usuarioIds?.includes(u.id) || a.usuarioId === u.id,
          );
    ...
    if (areaNorm === "médico" || areaNorm === "médica" || areaNorm === "qa") {
      ...
      row["Check Médico Por"] =
        users.find((u) => u.id === checklist?.medicaUserId)?.name || "";
      row["Check Médico Fecha"] = checklist?.medicaTimestamp || "";

      row["Check Estilo"] = checklist?.estilo ? "SÍ" : "NO";
      row["Check Estilo Por"] =
        users.find((u) => u.id === checklist?.estiloUserId)?.name || "";
      row["Check Estilo Fecha"] = checklist?.estiloTimestamp || "";

      row["Check Referencias"] = checklist?.referencias ? "SÍ" : "NO";
      row["Check Referencias Por"] =
        users.find((u) => u.id === checklist?.referenciasUserId)?.name || "";
      row["Check Referencias Fecha"] = checklist?.referenciasTimestamp || "";

      // Calculate time between checks if available
      row["Tiempo Médico -> Estilo (Horas)"] =
        checklist?.medicaTimestamp && checklist?.estiloTimestamp
          ? calculateWorkingTime(
              new Date(checklist.medicaTimestamp),
              new Date(checklist.estiloTimestamp),
              isFelipeAssigned,
            ).totalHours
          : "";

      row["Tiempo Estilo -> Referencias (Horas)"] =
        checklist?.estiloTimestamp && checklist?.referenciasTimestamp
          ? calculateWorkingTime(
              new Date(checklist.estiloTimestamp),
              new Date(checklist.referenciasTimestamp),
              isFelipeAssigned,
            ).totalHours
          : "";
    }
...
export const fixOklchForHtml2Canvas = (clonedDoc: Document) => {
  const originalElements = Array.from(document.getElementsByTagName("*")) as HTMLElement[];
  ...
  clonedElements.forEach((el, index) => {
    ...
    try {
      ...
      // Comprehensive list of properties that might contain colors or oklch
      const props = [
        "color",
        "backgroundColor",
        "borderColor",
      ...
      props.forEach((prop) => {
        ...
        if (
          ...
          // If it's a shadow, remove it. Else fallback to black or white.
          if (prop.includes("Shadow")) {
            el.style[prop as keyof CSSStyleDeclaration] = "none";
          } else {
            // Check if it's a background and use white, otherwise black
            const isBackground = prop.toLowerCase().includes("background");
            el.style[prop as keyof CSSStyleDeclaration] = isBackground
              ? "#ffffff"
              : "#000000";
          }
        } else if (value) {
      ...
      // Special handling for background images that might have gradients with oklch
  ...
  // Remove all stylesheets from the cloned document since we already inlined the computed styles.
  // This prevents html2canvas from crashing while trying to parse unsupported color functions like oklab/oklch.
